...
The various outputs generated by these scripts is described in [the project wiki](https://github.com/ONSdigital/geo-scripts/wiki).
...
This script derives parent geographies and successor geographies (for terminated areas) based on a point-in-polygon method. Calculating these relations "on the fly" avoids the need to download and maintain a large number of official lookup files. The output from this script is merged with the metadata generated in the previous step, and outputs the [master lookup](https://github.com/ONSdigital/geo-scripts/wiki/Master-lookup) file `/config/lookup/lookup.csv`.

### make-lists

This script generates [CSV lists of codes and names](https://github.com/ONSdigital/geo-scripts/wiki/Area-lists) for places that can be found via search in **Build a Custom Area Profile** and **Explore Local Statistics**. The metadata is extracted from the lookup file generated by the `make-relations` script.
...
This script generates the [area metadata files](https://github.com/ONSdigital/geo-scripts/wiki/Area-metadata-files) required for **Census Maps** (in the `/output/cm-geos` folder), and a common format shared by **Build a Custom Area Profile** and **Explore Local Statistics** (in the `/output/geos` folder). The config can be found at `/config/geo-config.js`.

**Important!** Make sure that the `/output/geos` folder is empty before running this script.

*Note: The .json output files in the output folder are actually gzipped to reduce file size, so cannot be opened directly. If you want to inspect their contents, you need to add .gz to their filename, and then uncompress them.*

### make-cm-geos

This optional script can be run to generate [area metadata files](https://github.com/ONSdigital/geo-scripts/wiki/Area-metadata-files#census-map-legacy-format) for [Census maps](https://www.ons.gov.uk/census/maps/). It relies on the output of the `make-geos` script.

### make-vtiles

This script generates (vector tiles)[https://github.com/ONSdigital/geo-scripts/wiki/Vector-tiles] for the most commonly used smaller geography types, including local authorities, wards and statistical geographies. The output is in the form of .mbtiles files, written to the `/output/vtiles` folder. The config can be found at `/config/vtiles-config.js`. ([Example output](https://stevage.github.io/vector-inspector/#?url=https://cdn.ons.gov.uk/maptiles/administrative/2023/authorities-all/v1/boundaries/8/127/83.pbf&loc=9/51.504/-0.1119)).
...
These two scripts are used in sequence to generate (vector tiles)[https://github.com/ONSdigital/geo-scripts/wiki/Vector-tiles] for output areas suitable for both high and low zoom levels, specifically designed for use in the original version of **Build a Custom Area Profile**. ([Example output](https://stevage.github.io/vector-inspector/#?url=https://cdn.ons.gov.uk/maptiles/administrative/2021/oa/v3/boundaries/8/127/83.pbf&loc=9/51.504/-0.1119)).

Whereas Tippecanoe is capable of merging together small areas to reduce the file size of tiles at lower zoom levels, these scripts explicitly merge the smallest OAs into their LSOA and MSOA parents, which is preferable both visually and in terms of the functionality of the above product.
...
These ZIP files are suitable for uploading and unzipping to serve from a static file service such as AWS S3, avoiding the need to run a tile server. (This is how vector tiles are currently served for ONS products.)

### make-postcodes

This script takes the [National Statistic Postcode Lookup](https://www.ons.gov.uk/methodology/geography/geographicalproducts/postcodeproducts) as an input and generates a series of [postcode lookup files](https://github.com/ONSdigital/geo-scripts/wiki/Postcode-lookup-files) with filenames in the form **/P.json**, **/PO.json**, **/PO1.json**, **/PO14.json**. The files contain postcodes with those prefixes and their longitude/latitude centroid coordinates.

### make-search-tiles

This script generates a directory of [tiled GeoJSON files](https://github.com/ONSdigital/geo-scripts/wiki/GeoJSON-tiles) that can be used for point-in-polygon "reverse geocoding" lookup to find areas that match given longitude/latitude coordinates. The directory structure follows a **{z}/{x}/{y}.json** pattern, equivalent to [web map tile coordinates](https://en.wikipedia.org/wiki/Tiled_web_map#Defining_a_tiled_web_map).
...
- Adding various additional metadata to the geo files, including codes to allow linkages to other products and API data sources.
